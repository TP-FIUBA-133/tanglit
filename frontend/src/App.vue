...
async function parse_slides(raw_markdown: string): Promise<number[]> {
  let rv = (await invoke(TANGLIT_COMMANDS.parse_slides, { raw_markdown })) as Array<{ start_line: number, tag: string }>;
  return rv.map((item) => item.start_line);
}

async function parse_blocks(raw_markdown: string): Promise<number[]> {
  let rv = (await invoke(TANGLIT_COMMANDS.parse_blocks, { raw_markdown })) as Array<{ start_line: number, tag: string }>;
  all_blocks.value = rv;
  return rv.map((item) => item.start_line);
}

async function execute_block(raw_markdown: string, block_name: string): Promise<string> {
  let rv = await invoke(TANGLIT_COMMANDS.execute_block, { raw_markdown, block_name }) as string;
  return rv;
}
