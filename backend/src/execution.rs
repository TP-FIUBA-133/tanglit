...
use crate::tangle::{get_codeblock, tangle_codeblock};
...
fn make_executable_code(
    ...
    for import in &code_block.imports {
        if let Some(import_block) = blocks.get(import) {
            // Tangle the imported block
            let import_output = tangle_codeblock(import_block, &blocks)?;
    ...
    let code = tangle_codeblock(code_block, &blocks)?;
...
/// # Arguments
/// * `blocks` - A HashMap containing all code blocks
...
pub fn execute(
    ...
    let block = get_codeblock(target_block, &blocks)?;
    ...
    let handles = match block.language {
        ...
        crate::parser::code_block::Language::Rust => {
            return Err(ExecutionError::UnsupportedLanguage("Rust".into()));
        }
        crate::parser::code_block::Language::Unknown(lang) => {
            return Err(ExecutionError::UnsupportedLanguage(lang));
        }
    };
...
mod tests {
    ...
    fn test_make_executable_code_c() {
        ...
        let tangle = make_executable_code(&main, blocks).unwrap();
        assert_eq!(
            tangle,
